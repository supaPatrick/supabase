...
Supabase Auth supports enterprise level Single Sign-On (SSO) using the SAML 2.0 protocol. In this document you will learn how you can manage connections to compatible identity providers.

<Admonition type="info">
This is an early beta release of these APIs. We are still working on adding support and a great developer experience in the CLI and Dashboard.
...
We do not believe these APIs will change significantly until the feature is generally available, but we do reserve the right to modify them. Projects in the beta will be notified of any changes.

</Admonition>

## Terminology

When working with SAML and SSO, it often can feel like there's too many acronyms to keep track of. Here's what all of them mean.

- **Identity Provider**, **IdP** or **IDP**  
  This is software that manages user accounts at a company or organization. It is able to verify the identity of a user and exchange that information with your Supabase project. Commonly used identity providers are: Microsoft Active Directory (Azure AD, Microsoft Entra), Okta, Google Workspaces (GSuite), PingIdentity, OneLogin, and many others.
- **Service Provider**, **SP**  
...
A common use case with SSO is to use the UUID of the identity provider as the identifier for the organization the user belongs to. This way you can use restrictive RLS policies to scope down the SQL actions and data that a user is able to access based on what SSO provider was used to sign the user in.
